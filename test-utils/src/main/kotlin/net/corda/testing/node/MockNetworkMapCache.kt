...
class MockNetworkMapCache : InMemoryNetworkMapCache() {
    ...
    init {
        val mockNodeA = NodeInfo(listOf(BANK_C_ADDR), setOf(BANK_C), MOCK_VERSION_INFO.platformVersion)
        val mockNodeB = NodeInfo(listOf(BANK_D_ADDR), setOf(BANK_D), MOCK_VERSION_INFO.platformVersion)
